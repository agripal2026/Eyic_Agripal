...
if os.path.exists(_sk_path):
    with open(_sk_path, 'rb') as _f: app.config['SECRET_KEY'] = _f.read()
else:
    _sk = os.urandom(32)
    with open(_sk_path, 'wb') as _f: _f.write(_sk)
...
def calculate_dosage(area, area_unit, pesticide_info, infection_pct=100.0):
    ...
    infection_pct (float, 1-100):
        The effective percentage of the total field that is infected.
        = max(AI-detected plant_severity, farmer-slider infection_percent)
        Dosage is scaled to only cover the infected area, not the whole field.
    """
    logger.info("="*60)
    ...
    try:
        ...
        # Clamp infection_pct to valid range
        ...
        # ── KEY CHANGE: scale by infection percentage ──────────────────────
        # Only the infected portion of the field needs treatment.
        # Severity multiplier adds a small extra margin for heavy infections.
        effective_hectares = hectare_conversion * (infection_pct / 100.0)

        if   infection_pct >= 75: severity_multiplier = 1.25   # Severe   — 25% extra
        elif infection_pct >= 50: severity_multiplier = 1.10   # Moderate — 10% extra
        elif infection_pct >= 25: severity_multiplier = 1.00   # Mild     — standard
        else:                     severity_multiplier = 0.85   # Early    — reduce dose

        logger.info(f"📊 Effective area (infected): {effective_hectares:.4f} ha")
        logger.info(f"📊 Severity multiplier: x{severity_multiplier}")

        # Calculate final dosages
        ...
        # Return hectare_conversion (total ha) so callers keep backward compat.
        # effective_area_hectares is logged and passed separately in the route.
...
Happy farming! 🌾🚜✨"""
    ...
    # For all other questions, use fallback rule-based response
    else:
...
def analyze_weekly_progress(user_id, plant_type, current_detection, detection_mode='continue'):
    ...
    detection_mode:
        'continue' - same plant as last week, increment week counter
        'new'      - fresh start for this plant, reset to week 1
    ...
    # If user chose NEW detection, treat this as a fresh start regardless of history
    ...
    # Get current session only (newest first, stop at week_number==1)
...
def init_database():
    with app.app_context():
        ...
        # Auto-migrate: add field_name, field_location, session_id if missing
        try:
...
def dashboard():
    ...
    # Weekly assessments: group by plant_type then split into sessions at week_number==1
    try:
        ...
        for _pt, _recs in raw_by_plant.items():
            ...
            for _r in _recs:
                _cur.append(_r)
                if _r.week_number == 1: _sessions.append(_cur); _cur = []
            if _cur: _sessions.append(_cur)
...
def post_harvest_page():
    """Render post-harvest management page, optionally auto-opening a section"""
    # Support ?section=shops  AND  ?service=shops&type=chemical (used by result/dashboard buttons)
...
@app.route('/schemes')
def schemes_page():
    """Redirect to post-harvest page, auto-open schemes section"""
...
def get_active_plant_session():
    """Return the most recent plant the user was tracking (last 60 days)."""
    try:
...
def get_plant_session_history():
    """Return all plants the user tracked in the last 90 days with progress summary."""
    try:
...
def set_plant_session():
    """
    Store detection mode in Flask session.
    detection_mode: 'continue' (same plant, continues weekly tracking)
                    'new'      (fresh plant, resets weekly counter)
    """
    try:
        data = request.get_json()
...
def get_weekly_history(plant_type):
    """Return full weekly assessment history for a plant (for progress display)."""
    try:
...
def analyze():
    ...
    try:
        ...
        # ── NEW: farmer-reported infection percentage from slider ──────────
        # Combined with AI-detected plant_severity in STEP 10.
        # We take max(user_reported, ai_detected) to avoid underestimating.
        try:
            infection_percent = float(request.form.get("infection_percent", 50))
            infection_percent = max(1.0, min(100.0, infection_percent))
        except (ValueError, TypeError):
            infection_percent = 50.0
        logger.info(f"🦠 Farmer-reported infection %: {infection_percent}")
        ...
        # ===== STEP 10: DOSAGE (infection-aware) =====
        disease_info = get_disease_info(predicted_class)
        chemical_dosage = organic_dosage = hectare_conversion = None
        effective_infection_pct = infection_percent   # default = farmer slider
        effective_area_hectares = 0.0
        area_in_hectares = 0.0

        if disease_info and area_float > 0:
            # ── Use the HIGHER of AI severity vs farmer-reported % ─────────
            # plant_severity comes from segment2.py (per-image leaf analysis)
            # infection_percent comes from the farmer's slider (field-level)
            effective_infection_pct = max(float(plant_severity), float(infection_percent))
            effective_infection_pct = max(1.0, min(100.0, effective_infection_pct))

            # ── Pass effective_infection_pct into dosage calculation ───────
            chemical_dosage, organic_dosage, hectare_conversion = calculate_dosage(
                area_float, area_unit, disease_info["pesticide"],
                infection_pct=effective_infection_pct
            )

            # Store for template rendering
            area_in_hectares     = hectare_conversion  # total hectares
            effective_area_hectares = round(area_in_hectares * (effective_infection_pct / 100.0), 4)

            logger.info(f"🦠 Effective infection %: {effective_infection_pct}  (AI={plant_severity}%, Farmer={infection_percent}%)")
            logger.info(f"📐 Total: {area_in_hectares:.4f} ha | Effective (infected): {effective_area_hectares:.4f} ha")
        ...
        # Always continue weekly tracking — no user prompt needed
        detection_mode = 'continue'
        logger.info(f"🔁 Auto-continuing weekly tracking for: {dominant_plant_type}")
        ...
        # Extract AI-recommended pesticide name from disease_info
        _rec_chem = _rec_org = 'Not specified'; _rec_dose = 0.0
        if disease_info and 'pesticide' in disease_info:
            _c = disease_info['pesticide'].get('chemical', {}); _o = disease_info['pesticide'].get('organic', {})
            if _c.get('name'): _rec_chem = _c['name']; _rec_dose = float(_c.get('dosage_per_hectare', 0) or 0)
            if _o.get('name'): _rec_org = _o['name']
        elif combined_treatments and 'pesticide' in combined_treatments:
            import re as _re
            for _tt, _dest in [('chemical', '_rec_chem'), ('organic', '_rec_org')]:
                _u = combined_treatments['pesticide'].get(_tt, {}).get('usage', '')
                _m = _re.search(r'\*\*(.+?)\*\*', _u)
                if _m:
                    if _tt == 'chemical': _rec_chem = _m.group(1)
                    else: _rec_org = _m.group(1)
        _farmer_p = (request.form.get('previous_pesticide') or '').strip()
        _p_type = request.form.get('pesticide_type', 'chemical')
        _pesticide_save = _farmer_p or (_rec_org if _p_type == 'organic' else _rec_chem)
        ...
        assessment_saved = save_weekly_assessment(
            ...
            dominant_plant_type,
            {
                ...
                'affected_percentage': plant_severity,
                'image_filename': image_filename,
                'pesticide_used': _pesticide_save,
                'pesticide_type': _p_type,
                'dosage_applied': float(request.form.get('dosage_used', 0) or 0) or _rec_dose,
        ...
        return render_template(
            ...
            # ── NEW: infection-aware dosage breakdown ─────────────────────
            infection_percent=infection_percent,
            plant_severity_pct=plant_severity,
            effective_infection_pct=effective_infection_pct,
            area_in_hectares=round(area_in_hectares, 4),
            effective_area_hectares=effective_area_hectares,
...
def calculate_dosage_api():
    ...
    Expected JSON body:
    {
        ...
        "area":            1200,
        "area_unit":       "square_meter",
        "infection_percent": 40,    # farmer slider value (1-100)
        "plant_severity":    35.5   # AI-detected severity from segment2 (0-100)
    }
    """
    try:
        data = request.json
        ...
        # Effective percentage = max of what farmer reports vs what AI detects
        ...
        if disease_info and 'pesticide' in disease_info:
            ...
            # Compute breakdown values for the frontend
            CONVERSIONS = {
            ...
            if   effective_pct >= 75: severity_mult = 1.25
            elif effective_pct >= 50: severity_mult = 1.10
            elif effective_pct >= 25: severity_mult = 1.00
            else:                     severity_mult = 0.85
...
# Migrate old uploads/ folder to static/expert_uploads/ if needed
...
@app.route("/logout")
def expert_logout():
    session.pop("admin", None)  # Only clear admin session, not user login session
    return redirect("/admin-login")
...
# Optional config — override by setting environment variables or editing agri_config.py
class _AgriConfig:
    # ── MySQL — reads from environment vars, falls back to agri_config.py defaults ──
    MYSQL_CONFIG = {
        'host':     os.environ.get('MYSQL_HOST',     'localhost'),
        'user':     os.environ.get('MYSQL_USER',     'root'),
        'password': os.environ.get('MYSQL_PASSWORD', 'pavan'),        # default from agri_config.py
        'database': os.environ.get('MYSQL_DATABASE', 'agriculture_llm'),  # from agri_config.py
        'port':     int(os.environ.get('MYSQL_PORT', 3306)),
    }

    # ── Vector DB — matches agri_config.py chroma_db folder ──
    VECTOR_DB_PATH  = os.environ.get('CHROMA_PATH', str(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'chroma_db')))

    # ── Embedding model — same as both config files ──
    EMBEDDING_MODEL = os.environ.get('EMBEDDING_MODEL', 'all-MiniLM-L6-v2')

    # ── Chunking settings (from agri_config.py) ──
    CHUNK_SIZE     = 800
    CHUNK_OVERLAP  = 100
    MIN_CHUNK_SIZE = 200

    # ── Crops — merged from agri_config.py CROP_CATEGORIES + config.py SUPPORTED_CROPS ──
    CROP_CATEGORIES = [
    ...
    # ── States — merged from both config files ──
    STATES = [
    ...
    # ── Seasons — merged from both config files ──
    SEASONS = ['Kharif', 'Rabi', 'Zaid', 'Zayad', 'Summer', 'Winter', 'Monsoon', 'Perennial', 'Annual']

    # ── RAG retrieval settings (from config.py) ──
    TOP_K_DOCUMENTS      = 5
    SIMILARITY_THRESHOLD = 0.3
    MAX_CONTEXT_TOKENS   = 8000

    # ── Intent keywords for smarter query classification (from config.py) ──
    INTENT_KEYWORDS = {
...
class AgricultureChatbot:
    ...
    def classify_query(self, query):
        ...
        # Use INTENT_KEYWORDS from config for smarter matching
    ...
    def get_market_prices(self, crop=None, state=None):
        ...
        params = []
        if crop:  sql += " AND crop_name LIKE %s"; params.append(f"%{crop}%")
        if state: sql += " AND state LIKE %s";     params.append(f"%{state}%")
    ...
    def search_documents(self, query, n_results=3, filters=None):
        ...
        try:
            ...
            if filters:
                conditions = []
                if filters.get('crop'):  conditions.append({'crop':  {'$eq': filters['crop']}})
                if filters.get('state'): conditions.append({'state': {'$eq': filters['state']}})
                if len(conditions) == 1: kwargs['where'] = conditions[0]
                elif len(conditions) > 1: kwargs['where'] = {'$and': conditions}
    ...
    def is_useful_chunk(self, text):
        ...
        if len(non_alpha) > len(t) * 0.4: return False
        lines = [l.strip() for l in t.splitlines() if l.strip()]
        if len(lines) <= 3 and all(l.isupper() or len(l) < 40 for l in lines): return False
        if not _re.search(r'[a-zA-Z]{4,}.*[a-zA-Z]{4,}', t): return False
        return True
    ...
    def close(self):
        if self.mysql_cursor: self.mysql_cursor.close()
        if self.mysql_conn:   self.mysql_conn.close()


# Initialize KisanAI chatbot (lazy — won't crash if deps missing)
