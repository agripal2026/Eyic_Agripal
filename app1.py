...
def calculate_dosage(area, area_unit, pesticide_info):
    ...
    try:
        ...
        try:
            ...
            logger.warning("Chemical dosage_per_hectare is not numeric (N/A or text) — defaulting to 0")
            chemical_dosage_per_hectare = 0.0

        try:
            organic_dosage_per_hectare = float(organic_info.get("dosage_per_hectare", 0) or 0)
        except (ValueError, TypeError):
            logger.warning("Organic dosage_per_hectare is not numeric (N/A or text) — defaulting to 0")
...
def analyze():
    ...
    try:
        ...
        # Read detection_mode: Flask session first, hidden form field as fallback
        detection_mode = session.get('detection_mode') or request.form.get('_detection_mode', 'continue')
        active_plant_from_session = session.get('active_plant_type') or request.form.get('_active_plant_type', '')
        logger.info(f"🔍 mode={detection_mode}, active={active_plant_from_session}, detected={dominant_plant_type}")
        if detection_mode == 'new':
            logger.info(f"🆕 New session: {dominant_plant_type}")
        elif active_plant_from_session and active_plant_from_session.strip().lower() != dominant_plant_type.strip().lower():
            detection_mode = 'new'
            logger.info(f"🆕 Mismatch → new: expected={active_plant_from_session}, got={dominant_plant_type}")
        else:
            detection_mode = 'continue'
            logger.info(f"🔁 Continuing: {dominant_plant_type}")
        session.pop('detection_mode', None)
        session.pop('active_plant_type', None)
...
if __name__ == '__main__':
    ...
    init_database()   # <-- THIS CREATES users TABLE
    ...
    logger.info(f"AI status: ❌ Not configured (rule-based mode)")
    logger.info("="*80)
